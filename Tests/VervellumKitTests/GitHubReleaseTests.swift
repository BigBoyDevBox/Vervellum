...
final class GitHubReleaseTests: XCTestCase {
    ...
    /// shared location like `/tmp` (a leftover `Vervellum.dmg` there made them flaky).
    private var tempDir: URL!

    override func setUpWithError() throws {
        tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("VervellumReleaseTest-\(UUID().uuidString)", isDirectory: true)
    ...
    {
      "tag_name": "v1.4.0",
      "name": "Vervellum 1.4.0",
      "body": "Notes here",
      "html_url": "https://github.com/L-K-M/Vervellum/releases/tag/v1.4.0",
      "prerelease": false,
      "draft": false,
      "published_at": "2026-06-01T12:00:00Z",
      "assets": [
        { "name": "Vervellum-1.4.0.zip", "content_type": "application/zip", "size": 1, "browser_download_url": "https://example.com/Vervellum.zip" },
        { "name": "Vervellum-1.4.0.dmg", "content_type": "application/x-apple-diskimage", "size": 2, "browser_download_url": "https://example.com/Vervellum.dmg" }
      ]
    }
    """.utf8)
    ...
    func testPrefersDMGAsset() throws {
        ...
        decoder.dateDecodingStrategy = .iso8601
        let release = try decoder.decode(GitHubRelease.self, from: json)
        XCTAssertEqual(release.preferredAsset?.name, "Vervellum-1.4.0.dmg")
    }
    ...
    func testUpdateDownloaderSanitizesAssetNames() throws {
        XCTAssertEqual(
            try UpdateDownloader.uniqueDestination(in: tempDir, fileName: "../../Vervellum.dmg").lastPathComponent,
            "Vervellum.dmg"
        )
        XCTAssertEqual(
            try UpdateDownloader.uniqueDestination(in: tempDir, fileName: "Vervellum\u{0000}.zip").lastPathComponent,
            "Vervellum-.zip"
        )
        XCTAssertEqual(
    ...
    func testUniqueDestinationSkipsExistingFilesAndDanglingSymlinks() throws {
        ...
        // With Vervellum.dmg and Vervellum-1.dmg taken, the -2/-3 suffixing kicks in.
        try Data().write(to: tempDir.appendingPathComponent("Vervellum.dmg"))
        try Data().write(to: tempDir.appendingPathComponent("Vervellum-1.dmg"))
        XCTAssertEqual(
            try UpdateDownloader.uniqueDestination(in: tempDir, fileName: "Vervellum.dmg").lastPathComponent,
            "Vervellum-2.dmg"
        )
        ...
        try FileManager.default.createSymbolicLink(
            at: tempDir.appendingPathComponent("Vervellum-2.dmg"),
    ...
    func testUniqueDestinationThrowsWhenAttemptCapIsExhausted() throws {
        try Data().write(to: tempDir.appendingPathComponent("Vervellum.dmg"))
        try Data().write(to: tempDir.appendingPathComponent("Vervellum-1.dmg"))
        try Data().write(to: tempDir.appendingPathComponent("Vervellum-2.dmg"))

        XCTAssertThrowsError(
            try UpdateDownloader.uniqueDestination(in: tempDir, fileName: "Vervellum.dmg", maxAttempts: 2)
        ) { error in
            ...
            XCTAssertEqual(fileName, "Vervellum.dmg")
    ...
    func testDownloadedAssetSizeValidation() throws {
        let file = tempDir.appendingPathComponent("Vervellum.dmg")
