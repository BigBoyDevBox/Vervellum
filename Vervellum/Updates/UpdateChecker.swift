...
final class UpdateChecker: ObservableObject {
    ...
    /// Dock icon to click and no way to reach it.
    ///
    /// No level override: a modal alert is at `.modalPanel` (8) and the research panel
    /// at `.floating` (3), so the alert is already on top. An app that pushed its
    /// overlay up to `.popUpMenu` (101) would need one, and would then be covering
    /// system menus to get it.
    @MainActor
