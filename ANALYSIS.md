...
code as of `main` (5be8eab).

## In flight (PRs open, do not redo)

Opened from the fork `BigBoyDevBox/Vervellum` against `main`. Each branch is
self-contained; merge order does not matter, except that #15 (tables) touches
`MarkdownParser` and everything else avoids it.

| PR | Branch | Scope |
| --- | --- | --- |
| #9 | `k3/atx-heading-closing-sequence` | Bug: `# C#` rendered as "C" — trailing `#` stripped without the required preceding space. Core + tests. |
| #10 | `k3/reject-fractional-citations` | Bug: `AssessmentParser` rounded a fractional source number (2.7) onto a real source (3). Core + tests. |
| #12 | `k3/streaming-render-throttle` | Perf: macOS republished per token (O(answer²) parsing per turn); now coalesces prose-only updates to 10 Hz, mirroring `LinuxPanel`. `ResearchEngine`. |
| #13 | `k3/ask-from-history` | Bug: asking while the history list was open ran the research invisibly behind it. `PanelRootView`. |
| #14 | `k3/scroll-follow` | Bug/UX: every token scrolled to bottom even while the user read back. Adds `BottomSentinel` + "Latest" pill; follows only while pinned. |
| #15 | `k3/markdown-tables` | Feature: pipe tables in `MarkdownParser` (delimiter-gated, `\|` escape, ragged rows normalised), `Grid` renderer on macOS, aligned `<tt>` in Pango. 8 tests. |
| #16 | `k3/turn-polish` | Feature: "Ask again" on completed turns; live elapsed clock in the process trail. |
| #17 | `k3/empty-state-examples` | Feature: clickable example questions seed the composer. |
| #18 | `k3/history-search-index` | Perf: history search lowercased every answer per keystroke; `ThreadSearchIndex` builds one haystack per thread on library change. Core + tests. |
| #19 | `k3/completion-signal` | Feature: menu-bar hourglass while running; soft sound when a run completes with the panel closed. |

Notes on verification: Core changes were tested locally on Linux (Swift 6.2.3,
190→197 tests green). macOS view changes could not be compiled locally (no Mac);
they are deliberately small and idiomatic. CI on the PRs needs maintainer
approval (first-time fork contributor), and the GLM review workflow skips fork
PRs by design.
...
- **P2 — `TurnView.validation` recomputes per render.** Mostly covered by the
  PR #12 throttle; if profiling still shows it, memoize per answer string.
