...
// Linux splits URLSession out of Foundation into its own module.
...
struct GitHubReleaseClient {
    ...
    private func fetch<T: Decodable>(_ type: T.Type, path: String) async throws -> T {
        ...
        // GitHub requires a User-Agent header. `AppIdentity` rather than the bundle,
        // so a Linux build — which has no bundle — identifies itself the same way.
